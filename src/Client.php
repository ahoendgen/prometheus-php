...
class Client
{
    ...
    public function pushMetrics(string $job = null, string $instance = null): void
    {
        ...
        if ($instance && !$job) {
            throw new PrometheusException('Instance passed but job was set to null.  Job must be set to a non empty string.');
        }
        if (null !== $job && '' == $job) {
            throw new PrometheusException('Job was set to an empty string.  Job must be set to a non empty string.');
        } elseif (null !== $instance && '' == $instance) {
            throw new PrometheusException('Instance was set to an empty string.  If Instance is set it must be a non empty string.');
        }
        if ($job) {
            $url .= $job;
        }
